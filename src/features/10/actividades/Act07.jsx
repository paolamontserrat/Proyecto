...
const Act07 = ({ data, onComplete, onBack, rango }) => {
    ...
    const columnas = config.columnas || ["CASA", "CAJA"];

    // Respuestas seleccionadas por el usuario: { [situacionId]: "CASA" | "CAJA" }
    ...
    const handleSelectOption = (id, columna) => {
            ...
            <div className="bg-white p-4 md:p-8 rounded-3xl border-4 border-alianza-amarillo shadow-2xl relative" translate="no">
                ...
                <div className="text-center mb-6">
                    <h1 className="font-extrabold text-blue-900 leading-tight text-2xl md:text-4xl tracking-wide uppercase">
                        {config.titulo || "MISIÓN: DETECTIVES DEL RIESGO"}
                    </h1>
                    <p className="text-gray-700 font-bold mt-2 text-base md:text-lg">
                        {config.instrucciones || "Lee cada situación y marca una X en la columna correcta:"}
                    </p>
                </div>
                    ...
                    <table className="w-full text-left border-separate border-spacing-y-2">
                        <thead>
                            <tr>
                                ...
                                {columnas.map((col) => (
                                    <th key={col} className="p-3 text-center font-black text-purple-900 text-lg md:text-2xl w-24 md:w-32">
                                        {col}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {situaciones.map((sit) => {
                                ...
                                return (
                                    <tr key={sit.id} className="bg-white rounded-2xl shadow-sm hover:shadow transition">
                                        <td className="p-3 md:p-4 rounded-l-2xl font-bold text-gray-800 text-sm md:text-base border-y border-l border-gray-100">
                                            {sit.texto}
                                        </td>
                                        {columnas.map((col, idx) => {
                                            const estaSeleccionado = seleccionUsuario === col;
                                            const esColumnaCorrecta = col === sit.respuestaCorrecta;
