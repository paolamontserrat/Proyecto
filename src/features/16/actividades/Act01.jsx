    ...
    return (
        <LayoutActividad fondo={config.fondo}>
            <style>{`
                ...
                /* Ajuste estricto para evitar que inputs de tipo fecha desborden en móviles */
            ...
            {/* Tarjeta principal con padding adaptativo para móvil */}
            <div className="bg-white p-4 sm:p-6 md:p-10 rounded-3xl border-4 border-alianza-amarillo shadow-2xl max-w-4xl mx-auto w-full box-border overflow-hidden" translate="no">
                
                {/* Título */}
                <div className="text-center mb-6 md:mb-8">
                    <h1 className="font-extrabold text-blue-900 leading-tight text-2xl sm:text-3xl md:text-5xl uppercase tracking-wide break-words">
                        {config.titulo || "META PRINCIPAL"}
                    </h1>
                    
                    {config.descripcion && (
                        <p className="text-gray-700 font-medium text-sm sm:text-base md:text-lg mt-3 max-w-2xl mx-auto leading-relaxed">
                            {config.descripcion}
                        </p>
                    )}

                    <div className="mt-4 sm:mt-6 inline-block bg-amber-100 border-2 border-amber-300 text-amber-900 px-4 sm:px-6 py-2 rounded-full font-bold text-sm sm:text-lg max-w-full">
                ...
                <div className="space-y-5 max-w-2xl mx-auto w-full">
                    {preguntas.map((p) => {
                        ...
                        return (
                            <div key={p.id} className="flex flex-col gap-1.5 w-full min-w-0">
                                <label className="font-extrabold text-blue-900 text-base sm:text-lg md:text-xl leading-snug break-words">
                                    {p.label}
                                </label>
                                ...
                                {esFecha ? (
                                    <div className="w-full min-w-0">
                                        <input
                                            ...
                                            className="w-full min-w-0 box-border bg-amber-50 border-2 border-amber-200 focus:border-amber-500 focus:bg-white rounded-2xl p-3 sm:p-4 text-gray-800 font-semibold text-sm sm:text-base md:text-lg outline-none transition-all shadow-inner cursor-pointer appearance-none"
                                        />
                                    </div>
                                ) : (
                                    <div className="w-full min-w-0">
                                        <input
                                            ...
                                            className={`w-full min-w-0 box-border rounded-2xl p-3 sm:p-4 font-semibold text-sm sm:text-base md:text-lg transition-all shadow-inner border-2 ${
                                                esCalculado
                ...
                <div className="mt-8 md:mt-10 max-w-3xl mx-auto space-y-6">

                    {/* Contenedor principal con fondo suave */}
                    <div className="bg-gradient-to-br from-amber-50 to-sky-50 rounded-3xl p-4 sm:p-6 md:p-8 border-2 border-amber-200/80 shadow-md">
                        
                        {/* Título destacado */}
                        {config.destacado && (
                            <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-amber-500 uppercase tracking-wide text-center mb-6 break-words">
                                {config.destacado}
                            </h2>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                            ...
                            <div className="bg-white rounded-2xl p-4 sm:p-5 border border-sky-200 shadow-sm relative">
                                <p className="text-sky-900 font-bold text-sm sm:text-base leading-relaxed whitespace-pre-line">
                            ...
                            {config.puntosClave && (
                                <div className="bg-white rounded-2xl p-4 sm:p-5 border border-amber-200 shadow-sm">
                                    <p className="font-extrabold text-blue-900 text-base sm:text-lg mb-2">A esta edad ya:</p>
                                    <ul className="space-y-2 text-gray-700 font-semibold text-sm sm:text-base">
                        ...
                        <div className="flex justify-center mt-6">
                            {imagenes[0] ? (
                                <img 
                                    ...
                                    className="w-36 sm:w-48 md:w-56 h-auto object-contain drop-shadow-xl animate-float-slow select-none"
                                />
                            ) : (
                                <div className="w-32 h-32 sm:w-40 sm:h-40 bg-amber-100 rounded-full flex items-center justify-center text-4xl sm:text-5xl animate-float-slow select-none shadow-md">
                                    💰
                                </div>
                            )}
                        </div>
