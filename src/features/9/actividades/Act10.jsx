...
// Suma los montos "ahorro" de todos los días del registro.
const sumarTotal = (registro) =>
  Object.values(registro).reduce(
    (suma, dia) => suma + (parseFloat(dia.ahorro) || 0),
    0,
  );
...
const Act10 = ({ data, onBack, onComplete, rango }) => {
  ...
  const isValidFirma = (d) => Array.isArray(d) && d.length > 0;

  // El total ya no vive en su propio state: se deriva del registro,
  // así nunca puede quedar desincronizado.
  const total = sumarTotal(registro);

  const isValid =
    Object.values(registro).every(
      (d) => d.accion.trim() !== "" && d.ahorro.trim() !== "",
    ) &&
    total > 0 &&
  ...
  const saveToSupabase = useCallback(
    async (registroGuardar, firmaGuardar) => {
      if (userId === "anon") return;

      setSyncStatus("saving");

      const firmaFinal = isValidFirma(firmaGuardar) ? firmaGuardar : [];
      const totalGuardar = sumarTotal(registroGuardar);
  ...
  const handleRegistro = (dia, campo, valor) => {
    const nuevo = {
      ...registro,
      [dia]: { ...registro[dia], [campo]: valor },
    };
    setRegistro(nuevo);
    scheduleSave(nuevo, firmaData);
  };
  ...
  const handleFirma = useCallback(
    (payload) => {
      ...
      setFirmaData(limpio);
      scheduleSave(registro, limpio);
    },
    [registro, scheduleSave],
  );

  const handleContinuar = async () => {
    if (!isValid) return;
    await saveToSupabase(registro, firmaData);
    onComplete();
  };
  ...
  return (
    <LayoutActividad fondo={data.recursos.fondo}>
      ...
      <div className="bg-white/95 rounded-[2rem] border-4 border-alianza-amarillo shadow-2xl p-6 md:p-10 animate-aparecer">
            ...
            >
              ...
              <div className="col-span-7">
                <input
                  ...
                  onChange={(e) => handleRegistro(key, "accion", e.target.value)}
              ...
              <div className="col-span-3">
                <input
                  ...
                  onChange={(e) => handleRegistro(key, "ahorro", e.target.value)}
        ...
        {/* TOTAL — ya no es un input, se calcula solo */}
        <div className="bg-green-100 rounded-3xl p-6 mb-8">
          ...
          <div className="max-w-sm mx-auto">
            <div className="w-full text-center text-3xl rounded-full border-4 border-green-400 py-3 font-black bg-white">
              ${total}
            </div>
          </div>

          <p className="text-center mt-5 text-lg">{data.total.mensaje}</p>
        </div>
        ...
        <div className="bg-orange-100 rounded-3xl p-6 mb-8">
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1">
              ...
              <p className="text-lg mb-4">{data.reconocimiento.descripcion}</p>

              <ul className="space-y-3">
                {data.reconocimiento.lista.map((item, index) => (
                  <li key={index} className="bg-white rounded-xl p-3 shadow">
                    ⭐ {item}
                  </li>
                ))}
              </ul>
        ...
        <div className="bg-sky-50 rounded-3xl p-8 shadow-lg">
          ...
          <p className="text-center text-lg mb-6">{data.firma.descripcion}</p>
        ...
        >
          {isValid ? data.botones.continuar : "Completa el registro para continuar"}
        </button>
      </div>
    </LayoutActividad>
  );
